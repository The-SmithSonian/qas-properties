...
(function () {
    ...
    // Uses hero.jpg if you upload one. Otherwise it uses the first
    // property photo, and the plain navy background if neither exists.

    let heroState = "loading";

    let heroFallbackUsed = false;

    function showHero(url, done) {
    ...
    function ensureHero() {
        ...
        if (heroState !== "missing" || heroFallbackUsed) return;

        const withPhoto = allProperties.find(function (property) {
    ...
    function loadHero() {
        ...
        showHero("hero.jpg", function (found) {

            heroState = found ? "ok" : "missing";
    ...
    async function loadProperties() {
        ...
        try {
            ...
            // Visitors are always treated as anonymous, never as the admin
    ...
    form.addEventListener("submit", function (event) {
        ...
        link.href = whatsappLink(buildEnquiry().text);
